...
class QuranLibrary {
  ...
  /// Stops all audio playback in QuranLibrary
  /// This method is used by AudioManager to coordinate between different audio systems
  Future<void> stopAudio() async {
    try {
      await AudioCtrl.instance.stopAllAudioPlayback();
    } catch (e) {
      log('QuranLibrary: Error stopping audio: $e');
    }
  }
  
  /// Sets up internal audio start callback for coordination
  /// This method is used by the main app to register for QuranLibrary internal audio start notifications
  static void setInternalAudioStartCallback(Function()? callback) {
    try {
      SurahState.setAudioStartCallback(callback);
      log('QuranLibrary: Internal audio start callback registered');
    } catch (e) {
      log('QuranLibrary: Error setting internal audio callback: $e');
    }
  }
  
  /// Sets the global audio player instance for QuranLibrary to use
  /// This ensures both QuranLibrary and main app use the same AudioPlayer instance
  static void setGlobalAudioPlayer(dynamic player) {
    try {
      SurahState.setGlobalAudioPlayer(player);
      log('QuranLibrary: Global audio player instance set successfully');
    } catch (e) {
      log('QuranLibrary: Error setting global audio player: $e');
    }
  }
